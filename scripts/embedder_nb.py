...
def create_embedder_nb(title:str, version:float, intro:str):
    EMBEDDER_NB = nbf.v4.new_notebook()
    EMBEDDER_NB.cells = [
        nbf.v4.new_markdown_cell(f"# {title} v{version} \n{intro}"),
        nbf.v4.new_markdown_cell(
            """
This notebook contains code walking through how to use `searchlite` with an {describe embedder type here}. This implementation requires {describe requirements}. Before running this notebook, **make sure you've pip installed the optional dependency** by doing:

```bash 
pip install <searchlite[dependency]>
```
...
sample_metadata = sample_df[["id", "category"]].to_dict(orient = "records")
        ...
        nbf.v4.new_markdown_cell("Now, you can instantiate the `Document` class. As shown below, both the text and metadata \
            are saved as attributes. Before performing search, you must generate embeddings for the texts stored within the \
                `Document` instance."),
        nbf.v4.new_markdown_cell(
            """We'll be using the {describe embedder type/model here} for this demo. Before writing any code, **{write any caveats or prereqs here}**.
...
{Write any automatic checks that the embedder does here}

To run an {embedder type}, you'll need to import `Document` and `<Embedder>` from `searchlite`.
"""
        ),
        nbf.v4.new_code_cell(
            """
from searchlite.document import Document
from searchlite.embedders import 
            """),
        nbf.v4.new_markdown_cell("Before creating your document, you have to instantiate your {embedder class}. You must pass the \
            {write any req args for the embedder here}. {Write any automatic checks that the embedder class does here}"),
        nbf.v4.new_code_cell("embedder = OllamaEmbedder(model_name = 'nomic-embed-text')"),
        nbf.v4.new_code_cell("embedder"),
        nbf.v4.new_markdown_cell(
            """
...
Be sure to assign your instantiated embedder to the embedder attribute of your document. If you don't, `searchlite` will automatically assign the `SkTFIDFEmbedder` as the embedding model for the document.
        ...
        nbf.v4.new_code_cell("doc = Document(texts = sample_texts, metadata = sample_metadata, embedder = embedder)"),
